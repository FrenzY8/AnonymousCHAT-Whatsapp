...
const { TUMBALPORT } = require('./src/settings.json');
...
const mulai = async (sesi, conn = new WAConnection()) => {
     ...
     conn.on('qr', async qr => {
          ...
          console.log(`[ ${moment().format('HH:mm:ss')} ] Scan ini untuk melanjutkan`)
          console.log(`will be restarted after 10 sec`)
     })
     ...
     conn.on('contacts-received', async () => {
          ...
          if (ConnectingWs) {
               conn.sendMessage(settings.Owner, isi, 'conversation', { quoted: { key: { remoteJid: "0@s.whatsapp.net", fromMe: false, }, message: { conversation: `*Anon BOT Beta! by FrenzYSG*`, }, } })
          }
     ...
     conn.on('open', (json) => {
          ...
          starting.start()
          if (ConnectingWs) {
               console.log(chalk.greenBright('[ Anonymous CHAT by FrenzYSG V1 ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.cyanBright('Connected as ' + json.user.name))
     ...
     wss.on('connection', async (ws) => {
          ...
          console.log(chalk.greenBright('[ Anonymous Message! ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.bgBlueBright('Client ' + userID + ' Connected '))
     ...
     conn.on('close', async ({ reason, isReconnecting }) => {
          if (!isReconnecting) {
               console.log(chalk.redBright('[WARN]  ') + chalk.yellow('Connect To Phone Rejected and Shutting Down.'))
               reconnect.stop()
               process.exit(1)
          }
     })
