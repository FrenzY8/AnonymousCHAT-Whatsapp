...
module.exports = handle = async (
     ...
     try {
          ...
          switch (cmd) {
               ...
               case 'P':
               case 'Help':               
                    balas(from, `${lang == 'id' ? `Halo ${pushname}👋, berikut perintah FrenzY X BOT untuk Anonymous Chat!` : `Hi ${pushname}👋, following the anonymous chat bot command`}
...
💌 Follow Me On Instagram - _${lang == 'id' ? 'Instagram : @frenzy.apk' : 'instagram : @frenzy.apk'}_
          ...
          `)
               ...
               case prf + 'search':
                    if (!isMatched(sender).status) {
                         const strMatch = lang == 'id' ? `\`\`\`Tunggu Sebentar, Bot sedang Mencarikan anda Teman untuk Mengobrol!\`\`\` 🔎` : `\`\`\`Please wait looking for chat friends\`\`\` 🔎\n\ntype: *${prf}stop* (to stop session)`
                         balas(from, strMatch)
                    }
                    match(sender)
                         .then(() => {
                              balasNp(from, lang == 'id' ? `\`\`\`Selamat! teman ditemukan! cobalah Untuk memberi Salam untuk Mengawali Chat nya :)\`\`\` ✅\n\nketik :\n*${prf}stop* ( untuk berhenti chat )\n*${prf}skip* ( untuk melewati chat ini )` : `Successfully found friends ✅\n\ntype:\n*${prf}stop* (to stop chatting)\n*${prf}skip* (to skip this chat)`)
                         })
                         .catch((error) => {
                              if (error.message == 'Sudah masuk sesi match' || error.message == 'sesi telah dimulai') {
                                   ...
                                   console.log(teks);
                                   balasNp(from, lang == 'id' ? `\`\`\`Tidak dapat menemukan teman! mungkin tidak ada yang online saat ini :(\n cobalah untuk mengundang teman anda Bermain bot ini :)\`\`\`\n\n❌ atau anda bisa mencari teman lagi (dengan mengulangi command)` : `Cannot find friends ❌ please search again!`)
                              }
                         })
                    break
               case prf + 'skip':
               ...
               case prf + 'stop':
                    stop(sender)
                         .then((data) => {
                              data.db.splice(data.index, 1)
                              fs.writeFileSync('./src/database/client-user.json', JSON.stringify(data.db, null, 5))
                              balas(from, lang == 'id' ? `\`\`\`Kamu telah memberhentikan sesi chat kamu dengan teman mu!\`\`\` ❌` : `\`\`\`This chat session has been suspended\`\`\` `)
                              if (data.target !== null) balasNp(data.target, lang == 'id' ? `\`\`\`Sesi chat ini telah diberhentikan oleh teman chat kamu\`\`\` ❌` : `\`\`\`This chat session has been suspended by your friend\`\`\` `)
                         })
                         .catch((error) => {
                              console.log(error);
